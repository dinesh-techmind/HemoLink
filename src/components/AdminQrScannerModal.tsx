  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Tab Navigation Controls (when not showing result) */}
        {!verificationResult && (
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-4">
          {/* ======================= CASE 1: VERIFICATION RESULT VIEW ======================= */}
          {verificationResult ? (
            <div className="space-y-4 animate-fadeIn">
              {/* Notice Banner if donation was logged */}
              ...
              <div
                className={`p-4 rounded-2xl border flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${
                  verificationResult.isValid
                    ? "bg-gradient-to-r from-emerald-950/50 via-card-dark to-emerald-950/30 border-emerald-500/50"
                    : "bg-gradient-to-r from-red-950/50 via-card-dark to-red-950/30 border-red-500/50"
                }`}
              >
                <div className="flex items-center gap-3">
                  <div
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      >
                        {verificationResult.isValid ? "VERIFIED OFFICIAL DONOR PASS" : "UNVERIFIED OR INVALID PASS"}
                      </span>
                      <span className="text-[10px] font-mono text-text-subtle">
                        {verificationResult.verificationTimestamp}
                      </span>
                    </div>
                    <h4 className="text-lg font-bold font-display text-text-bright mt-0.5">
                      {verificationResult.name}
                    </h4>
                    <p className="text-xs text-text-muted font-mono">
                      Pass ID: <span className="text-amber-400 font-bold">{verificationResult.donorId}</span>
                      {verificationResult.securityHash && (
                        <span className="text-text-subtle ml-2 hidden sm:inline">
                          Seal: {verificationResult.securityHash.slice(0, 16)}...
                        </span>
                      )}
                    </p>
                  </div>
                ...
                {/* Rescan Button */}
                <button
                  id="admin-rescan-pass-btn"
                  onClick={() => {
                    setVerificationResult(null);
                    setDonationLoggedNotice(null);
                    setActiveTab("camera");
                  }}
                  className="px-3.5 py-2 bg-amber-500/15 hover:bg-amber-500 border border-amber-500/40 text-amber-300 hover:text-white rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer shrink-0"
                >
                    ...
                    <div className="flex justify-between text-text-muted">
                      ...
                      <span
                        className={`font-bold font-mono text-[10px] ${
                          verificationResult.status === "AVAILABLE" ? "text-emerald-400" : "text-amber-400"
                        }`}
                      >
                        {verificationResult.status}
                      </span>
                    </div>
                    {verificationResult.matchedDonor?.phone && (
                      <div className="flex justify-between text-text-muted">
                        <span>Phone</span>
                        <span className="font-mono text-text-bright">{verificationResult.matchedDonor.phone}</span>
                      </div>
                    )}
                  </div>
                </div>
                    ...
                    {donors.slice(0, 4).map((d) => {
                      ...
                      return (
                        <div
                          ...
                          className="bg-surface-dark border border-border-dark hover:border-amber-500/50 p-3.5 rounded-2xl cursor-pointer transition space-y-2 group"
                        >
                          <div className="flex items-center justify-between">
                            ...
                            <div className="bg-white p-1 rounded-md shrink-0 shadow-sm">
                          ...
                          <div className="flex items-center justify-between pt-1 border-t border-border-dark/60 text-[10px]">
                            ...
                            <span className="text-amber-400 font-bold flex items-center gap-0.5 group-hover:translate-x-0.5 transition">
                              Simulate Scan <ChevronRight className="w-3 h-3" />
                            </span>
                          </div>
                        </div>
                      );
              ...
              {activeTab === "manual" && (
                <div className="space-y-4">
                  <div className="bg-surface-dark p-5 rounded-2xl border border-border-dark space-y-3">
                    ...
                    <div className="pt-2">
                      <span className="text-[10px] text-text-subtle block mb-1 font-mono">Quick Match Shortcuts:</span>
                      <div className="flex flex-wrap gap-1.5">
                        {donors.slice(0, 3).map((d) => (
                          <button
                            ...
                            className="text-[10px] font-mono bg-card-dark hover:bg-zinc-800 border border-border-dark text-text-bright px-2 py-1 rounded-lg transition cursor-pointer"
                          >
                            {formatDonorId(d)} ({d.fullName})
                          </button>
                        ))}
                      </div>
                    </div>
